...
def get_auth_token(url=''):
    logger.debug("get_auth_token url='%s'" % url)
    token = ''
    try:
        resp, content = httplib2.Http().request(url, method='GET', body='', headers={})
        status, resp_body = resp['status'], content.decode('UTF-8')
        if status == '200':
            token = 'Bearer ' + json.loads(resp_body)['result']
            # token = resp_body['result']
    except BaseException as x:
        logger.debug("get auth token exception: %s" % x)
    return token


def call_req(base_url, user, passwd, auth_type, resource, method, content='', get_auth_token_url=''):
    callid = str(uuid.uuid1())
    logger.debug("[%s]call_req('%s','%s','%s',%s,'%s','%s','%s', '%s')" % (
        callid, base_url, user, passwd, auth_type, resource, method, content, get_auth_token_url))
    ret = None
    resp_status = ''
    try:
        ...
        if user:
            headers['Authorization'] = 'Basic %s' % ('%s:%s' % (user, passwd)).encode("base64")
        if get_auth_token_url != '':
            headers['Authorization'] = get_auth_token(get_auth_token_url)
        ca_certs = None
...
def call_req_aai(base_url, user, passwd, auth_type, resource, method, content=''):
    ...
    try:
        full_url = combine_url(base_url, resource)
        headers = {'content-type': 'application/json', 'accept': 'application/json',
                   'X-FromAppId': 'VFC-SVNFM-certus-DRIVER', 'X-TransactionId': str(uuid.uuid1())}
        if user:
