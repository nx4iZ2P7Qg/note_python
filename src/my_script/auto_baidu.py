    ...
    while True:
        ...
        with open('nohup.out', 'r', encoding='utf-8') as f:
            ...
            if len(lines) > 0 and lines[len(lines) - 1].startswith('任务结束'):
                break
        # 启动
        subprocess.run(f'nohup ./BaiduPCS-Go d {path} --save --locate -p 1 -l 1 &', shell=True)
        # 休眠1小时
        time.sleep(60 * 60)
    print("job finished")
    # 移动nohup.out文件
    file_name_old = 'nohup.out'
    file_name_new = file_name_old + '.' + path[path.rindex('/') + 1:]
    subprocess.run(f'mv {file_name_old} {file_name_new}', shell=True)


self_continue_download('/df/Textractor.7z')
